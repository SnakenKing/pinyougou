...
public class AddressController {
    ...
    public List<TbAddress> findAddressListByUserId() {
        //1.获取当前用户的用户的名称(user_id)
        String userId = SecurityContextHolder.getContext().getAuthentication().getName();
        //2.调用用户中心服务的方法获取该用户下的所有的地址的列表
        TbAddress condition = new TbAddress();//是等号的条件 select * from tb_address where user_id =?
        condition.setUserId(userId);
        List<TbAddress> addressList = addressService.select(condition);
