...
public class SeckillGoodsServiceImpl extends CoreServiceImpl<TbSeckillGoods> implements SeckillGoodsService {
    ...
    public Map getGoodsById(Long id) {
        ...
        Date endTime = tbSeckillGoods.getEndTime();//结束时间
        Integer stockCount = tbSeckillGoods.getStockCount();//剩余库存
        long shengyu = endTime.getTime() - new Date().getTime();
        Map objectObjectHashMap = new HashMap<>();
        objectObjectHashMap.put("count", stockCount);//库存
        objectObjectHashMap.put("time", shengyu);//距离结束的毫秒数
