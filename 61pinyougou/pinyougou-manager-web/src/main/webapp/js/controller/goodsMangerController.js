var app = new Vue({
    ...
    methods: {
        ...
        //查询所有的商品分类的列表 数据 存储到数组中 下标就是ID 值就是分类名
        findAllItemCategory:function () {
            axios.get('/itemCat/findAll.shtml').then(
                function (response) {

                   var  itemCatListx = response.data;//List<itemcat>
                    //itemCatListx
                    for(var i=0;i<itemCatListx.length;i++){
                        var obj = itemCatListx[i];//{id,name}
                        app.itemCatList[obj.id]=obj.name;
                    }
        ...
        updateStatus:function (status) {
            axios.post('/goods/updateStatus/'+status+'.shtml',this.ids).then(
                function (response) {
                    if(response.data.success){
                        app.searchList(1);
                    }
